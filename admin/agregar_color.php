...
// Si viene un ID por GET, es edición
if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $res = $pdo->prepare("SELECT * FROM ventasbasica_colores WHERE id=?");
    $res->execute([$id]);
...
if (isset($_GET['eliminar'])) {
    ...
    $stmt = $pdo->prepare("DELETE FROM ventasbasica_colores WHERE id=?");
    $stmt->execute([$id]);
   echo "<script>
window.location='agregar_color.php';
</script>";
    exit;
}
...
if (isset($_POST['guardar'])) {
    ...
    if ($nombre != "" && preg_match('/^#[A-F0-9]{6}$/i', $hex)) {
        $stmt = $pdo->prepare("INSERT INTO ventasbasica_colores(nombre, hex, activo) VALUES (?, ?, ?)");
        $stmt->execute([$nombre, $hex, $activo]);
     echo "<script>
        window.location='agregar_color.php';
        </script>";
        exit;
...
if (isset($_POST['actualizar'])) {
    ...
    if ($nombre != "" && preg_match('/^#[A-F0-9]{6}$/i', $hex)) {
        $stmt = $pdo->prepare("UPDATE ventasbasica_colores SET nombre=?, hex=?, activo=? WHERE id=?");
        $stmt->execute([$nombre, $hex, $activo, $id]);
       echo "<script>
        window.location='agregar_color.php';
        </script>";
        exit;
...
// --- LISTADO DE COLORES ---
$colores = $pdo->query("SELECT * FROM ventasbasica_colores ORDER BY id DESC");
?>
<!DOCTYPE html>
<html lang="es">
...
<h3 class="mb-4"><?php echo $color['id'] ? 'Editar Color' : 'Nuevo Color'; ?></h3>
  ...
  <form method="POST" class="mb-5">
    ...
    <div class="mb-3">
      ...
      <input type="text" name="nombre" class="form-control" value="<?php echo htmlspecialchars($color['nombre']); ?>" required>
    </div>

    <div class="mb-3">
      <label>Código HEX:</label>
      <div class="color-input-group">
        <input type="color" id="colorPicker" value="<?php echo htmlspecialchars($color['hex']); ?>">
        <input type="text" name="hex" id="hexInput" class="form-control" maxlength="7"
               value="<?php echo htmlspecialchars($color['hex']); ?>"
               pattern="^#([A-Fa-f0-9]{6})$" required>
      </div>
      <small class="text-muted">Ejemplo: #FF0000</small>
    </div>

    <div class="form-check mb-3">
      <input class="form-check-input" type="checkbox" name="activo" id="activo" <?php echo $color['activo'] ? 'checked' : ''; ?>>
      <label class="form-check-label" for="activo">Activo</label>
    </div>

    <?php if ($color['id']) { ?>
      <button type="submit" name="actualizar" class="btn btn-primary">Actualizar</button>
      <a href="agregar_color.php" class="btn btn-secondary">Cancelar</a>
    <?php } else { ?>
  ...
  <table class="table table-bordered table-hover bg-white mt-3">
    ...
    <td data-label="Código HEX">
        <?php echo htmlspecialchars($c['hex']); ?>
    </td>

    <td data-label="Vista">
        <div class="color-preview" style="background-color: <?php echo htmlspecialchars($c['hex']); ?>;"></div>
    </td>
    ...
    <td data-label="Acciones">
        <a href="agregar_color.php?id=<?php echo $c['id']; ?>" class="btn btn-sm btn-primary">
            Editar
        </a>

        <a href="agregar_color.php?eliminar=<?php echo $c['id']; ?>"
...
// Sincroniza color picker y texto HEX
