...
// Ventas
$stmt_ventas = $pdo->prepare("SELECT IFNULL(SUM(total),0) total FROM ventasbasica_envios WHERE YEAR(fecha)=?");
$stmt_ventas->execute([$anio]);
$ventas = $stmt_ventas->fetch();


// Ganancia
$ganancia = $pdo->query("
SELECT IFNULL(SUM(ganancia),0) total
FROM ventasbasica_envios
")->fetch();
...
// Prendas
$prendas = $pdo->query("
SELECT IFNULL(SUM(cantidad_prendas),0) total
FROM ventasbasica_envios
")->fetch();
...
while($fila = $stmt_ventasMes->fetch()){
...
ORDER BY total DESC
");
$stmt_ventasDepto->execute([$anio]);

$departamentos = [];
$ventasDepartamento = [];

while($fila = $stmt_ventasDepto->fetch()){

    $departamentos[] = $fila['departamento'];
    $ventasDepartamento[] = $fila['total'];

}

//=======================================
// VENTAS POR RED SOCIAL
//=======================================
...
while($fila = $ventasRed->fetch()){

    $redes[] = $fila['red_social'];
    $ventasRedSocial[] = $fila['total'];
...
Q <?=number_format($ventas['total'],2)?>
...
Q <?=number_format($ganancia['total'],2)?>
...
<div class="card shadow border-0 mt-4">
    ...
    <div class="card-body">
        ...
        <canvas id="departamentosChart" height="130"></canvas>
...
<div class="card shadow border-0 mt-4">
    ...
    <div class="card-body">
        ...
        <canvas id="redesChart" height="120"></canvas>
 ...
 <div class="text-center mt-3">
        <a href="envios.php" class="btn btn-secondary">⬅️ Volver a envíos</a>
    </div>
...
const ctxDepto = document.getElementById('departamentosChart');
...
data:<?=json_encode($ventasDepartamento)?>,
...
const ctxRed = document.getElementById('redesChart');
...
labels:<?=json_encode($redes)?>,

datasets:[{

label:'Ventas',

data:<?=json_encode($ventasRedSocial)?>,
