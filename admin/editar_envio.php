...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header("Location: tu_archivo_principal.php");
    exit();
}
...
// Obtener datos
$stmt = $pdo->prepare("SELECT * FROM ventasbasica_envios WHERE id=?");
$stmt->execute([$id]);
$envio = $stmt->fetch();
...
if (!$envio) {
    echo "<script>
window.location='envios.php?ok=editado';
</script>";
    exit();
}
...
if (isset($_POST['actualizar'])) {
    ...
    $sql = "UPDATE ventasbasica_envios SET 
        fecha=?,
        numero_guia=?,
        cliente=?,
        estado=?,
...
<div class="container py-5">
    ...
    <div class="card shadow">
        <div class="card-header bg-primary text-white">
            <i class="fa fa-edit"></i> Editar Envío
        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="estado" class="form-select">
                            ...
                            <option <?=$envio['estado']=='Entregado'?'selected':''?>>Entregado</option>
                            <option <?=$envio['estado']=='Devolución'?'selected':''?>>Devolución</option>
                        </select>
                    </div>
    ...
    <label>Departamento</label>
            ...
            <select name="departamento" class="form-select">
                ...
                foreach ($departamentos as $dep) {
                    $selected = ($envio['departamento'] == $dep) ? 'selected' : '';
                ...
                ?>
                    ...
                    <div class="col-md-4 mb-3">
                       ...
                       <input type="number" class="form-control" name="cantidad"
                        value="<?=htmlspecialchars($envio['cantidad_prendas'] ?? '')?>">
                    </div>

                    <div class="col-md-4 mb-3">
                        <label>Inversión</label>
                        <input type="number" step="0.01" name="inversion" class="form-control"
                        value="<?=$envio['inversion']?>">
                    </div>

                    <div class="col-md-6 mb-3">
                        <label>Ganancia</label>
                        <input type="number" step="0.01" name="ganancia" class="form-control"
                        value="<?=$envio['ganancia']?>">
                    </div>

                    <div class="col-md-3 mb-3">
                        <label>Red Social</label>
                        <select name="red_social" class="form-select">
                            <option <?=$envio['red_social']=='Facebook'?'selected':''?>>Facebook</option>
                            <option <?=$envio['red_social']=='Instagram'?'selected':''?>>Instagram</option>
                            <option <?=$envio['red_social']=='Twitter'?'selected':''?>>Twitter</option>
                            <option <?=$envio['red_social']=='Pagina Web'?'selected':''?>>Pagina Web</option>
                            <option <?=$envio['red_social']=='WhatsApp'?'selected':''?>>WhatsApp</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-3 mb-3">
                        ...
                        <select name="empresa" class="form-select">
                            <option <?=$envio['empresa']=='Cargo Express'?'selected':''?>>Cargo Express</option>
                            <option <?=$envio['empresa']=='Box Delivery'?'selected':''?>>Box Delivery</option>
                        </select>
                    </div>

                </div>

                <button type="submit" name="actualizar" class="btn btn-success w-100 mt-3">
                    <i class="fa fa-save"></i> Actualizar Envío
                </button>
    ...
    <div class="text-center mt-3">
        <a href="envios.php" class="btn btn-secondary">⬅️ Volver a administracion</a>
    </div>
